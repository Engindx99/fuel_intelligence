...
def compute_clinker_kinetics_numba(Ts, X, m_CaO, m_SiO2, m_C2S, m_C3A, m_C4AF,
                                  ...
                                  pre_factors):
    """
    YAML hiyerarşisine tam uyumlu Numba çekirdeği.
    """
    N = len(Ts)
    ...
    for i in range(N):
        ...
        if Ts[i] >= T_min_vec[0] and X[i] < 1.0:
            ...
            rates[0, i] = np.minimum(k_calc * (1.0 - X[i]), 0.010)

        # 2. BELİT (C2S) OLUŞUMU
        if Ts[i] >= T_min_vec[1] and m_CaO[i] > 1e-6 and m_SiO2[i] > 1e-6:
            # %75 Kalsinasyon şartı (Soft-lock)
        ...
        if Ts[i] >= T_min_vec[2] and m_CaO[i] > 1e-6 and m_C2S[i] > 0.14:
            ...
            rates[2, i] = np.minimum(k_c3s * m_C2S[i] * m_CaO[i] * liquid_effect * lock_c3s, 0.015)

        # 4. YARDIMCI FAZLAR (C3A ve C4AF)
        if Ts[i] >= 1350.0:
            rates[3, i] = pre_factors[0] * (Ts[i] - 1350.0) # C3A flux
            rates[4, i] = pre_factors[1] * (Ts[i] - 1300.0) # C4AF flux
...
class CalcinationKinetics:
    def __init__(self, config):
        ...
        self.k0 = np.array([
            ...
            float(k_cfg.get('k0_c3s', 1.2e5))      # Alit
        ])
        
        self.Ea = np.array([
            float(k_cfg.get('Ea', 1.78e5)),
            float(k_cfg.get('Ea_c2s', 2.45e5)),
            float(k_cfg.get('Ea_c3s', 2.60e5))
        ])
        
        self.T_min = np.array([
            float(k_cfg.get('T_min_rxn', 1073.0)), # CaCO3 -> 1073K (800C)
