...
def _numba_step_core(N, Ts, Tg, Tw, X, m_CaO, m_SiO2, m_C2S, rho_g, dt, dz, v_s, m_dot_s, m_dot_g_inlet, 
    ...
    # Katı Sıcaklık Değişimi (Advection + Net Isı)
...
class KilnSolver:
    ...
    def solve_step(self, dt, fuel_rate=None, feed_rate=None, kiln_rpm=None, fan_rate=None):
        ...
        effective_thermal_mass = (m_dot_s / max(0.001, v_s)) * self.dz + 20.0
        ...
        # 3. Numba Çekirdek Çözümü (Termodinamik ve Ana Reaksiyonlar)
        ...
        # 4. Akışkan Fazlar (C3A ve C4AF) - Sinterleşme Bölgesi
        ...
        # 5. Tam Stokiyometrik Denge (Oksit Tüketimi Dahil)
        ...
        for p in phases:
            ...
            limit = 0.85 if p == 'm_C3S' else 0.45 if p == 'm_C2S' else 1.0
