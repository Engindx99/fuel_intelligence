...
class KilnState:
    def __init__(self, N):
        ...
        self._Ts_history = []
        self._Tg_history = []
        self._CO2_history = [] # Görselleştirme hatasını yakalamak için eklendi
        self._CaCO3_history = []
        self._CaO_history = []
        self._C2S_history = []
        self._C3S_history = []
    ...
    def total_solid_fraction(self):
        """Kütle korunumunu kontrol etmek için tüm katı fazların toplamı."""
        return (
    ...
    def initialize_profiles(self, config, raw_meal_comp=None):
        ...
        # Gaz sıcaklığı fırın boyunca lineer bir tahminle başlar
        self.Tg = np.linspace(T_ambient + 100.0, T_gas_max, self.N)

        if raw_meal_comp is None:
            # Config'deki varsayılan besleme kompozisyonu
            raw_meal_comp = config.get("feed", {}) # 'raw_meal_composition' yerine 'feed' daha yaygın

        # Normalize edilmiş başlangıç kütle fraksiyonları
        total_sum = sum(raw_meal_comp.values())
        if total_sum > 0:
            for key, val in raw_meal_comp.items():
                if hasattr(self, key):
                    # Tüm fırını başlangıçta ham madde ile dolduruyoruz
                    getattr(self, key).fill(val / total_sum)

        # Ürün fazlarını sıfırla (Başlangıçta sadece ham madde var)
        self.CaO.fill(0.0)
        self.C2S.fill(0.0)
        self.C3S.fill(0.0)
        self.C3A.fill(0.0)
    ...
    def snapshot(self, t):
        """Anlık durumu geçmişe kaydeder. Bellek yönetimi için kopyalar alınır."""
        ...
        self._CO2_history.append(self.CO2.copy()) # CO2 takibi kritik
    ...
    # --- Accessors (Grafik çizimi için kolaylık sağlar) ---
    @property
    def time_history(self): return np.array(self._time_history)
    
    @property
    def Ts_history(self): return np.array(self._Ts_history)
    
    @property
    def Tg_history(self): return np.array(self._Tg_history)

    @property
    def CO2_history(self): return np.array(self._CO2_history)

    @property
    def CaCO3_history(self): return np.array(self._CaCO3_history)

    @property
    def CaO_history(self): return np.array(self._CaO_history)

    @property
    def C2S_history(self): return np.array(self._C2S_history)

    @property
    def C3S_history(self): return np.array(self._C3S_history)