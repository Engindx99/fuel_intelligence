...
class StepExecutor:
    ...
    def perform_step(self, x: KilnState, t: float, inputs: dict = None) -> KilnState:
        ...
        x_next["Lignite_Coal"] = max(
            0.0, 1.0 - x_next["Petcoke"] - x_next["Alternative_Fuel"]
        )
        ...
        if "kiln_rpm" in inputs:
            ...
            alpha = 0.005
            x_next["kiln_rpm"] = max(
                0.1, rpm_current + alpha * (rpm_setpoint - rpm_current)
            )
        ...
        x_next["Material_acc"] = mat_acc + (x_next["Feed_rate"] / 1.55 - kiln_out) * (
            self.dt / 60.0
        )
