...
class StepExecutor:
    ...
    def perform_step(self, x: KilnState, t: float, inputs: dict = None):
        ...
        # ==========================================================
        # ENERGY INPUT (DIRECT ENTHALPY SOURCE)
        # ==========================================================

        Q_burn_in = Q_in_total  # kW (kJ/s)
        ...
        # ======================================================
        # CALCINATION ZONE (UNIT-CONSISTENT VERSION)
        # ======================================================

        h_gs, A_s = 320.0, 950.0
        UA = h_gs * A_s

        m_solid_calc = x_si.Feed_rate_kgs  # <-- FIX (was inconsistent)
        ...
        # ======================================================
        # ENERGY INPUT (CONSISTENT ENTHALPY FLOW)
        # ======================================================
        ...
        Q_in = Q_from_burning + Q_from_tertiary
        Q_in = np.tanh(Q_in / 5e6) * 5e6

        # ======================================================
        # HEAT TRANSFER
        # ======================================================

        dT = Tg_calc_curr - Ts_calc_curr
        Q_gs = UA * dT
        ...
        C_solid_calc = m_solid_calc * Cp_solid_calc
        C_gas_calc = m_gas_total * Cp_air + 1e-6

        # ======================================================
        # TIME CONSTANTS
        # ======================================================

        tau_s = C_solid_calc / (UA + 1e-9)
        tau_g = C_gas_calc / (UA + 1e-9)

        alpha_s = self.dt / (self.dt + tau_s)
        alpha_g = self.dt / (self.dt + tau_g)

        # ======================================================
        # SOLID ENERGY BALANCE
        # ======================================================

        Ts_eq = Tg_calc_curr + (Q_in / (UA + 1e-9))

        Ts_calc_pred = (1 - alpha_s) * Ts_calc_curr + alpha_s * Ts_eq

        # ======================================================
        # GAS ENERGY BALANCE
        # ======================================================

        Tg_eq = Ts_calc_pred + (Q_from_burning / (C_gas_calc + 1e-9))

        Tg_calc_pred = (1 - alpha_g) * Tg_calc_curr + alpha_g * Tg_eq

        # ======================================================
        # SOFT LIMIT (STABILITY ONLY, NO HARD CLIP)
        # ======================================================

        def soft_limit(T, T_min=400.0, T_max=1600.0, sharp=40.0):
            return T_min + (T_max - T_min) / (1 + np.exp(-(T - 1000.0) / sharp))

        Ts_calc_pred = soft_limit(Ts_calc_pred)
        Tg_calc_pred = soft_limit(Tg_calc_pred)
        ...
        # ==========================================================
        # CALCINATION (PHYSICALLY CONSISTENT + SI SAFE VERSION)
        # ==========================================================
        ...
        # ----------------------------------------------------------
        # SMOOTH ACTIVATION (dimensionless)
        # ----------------------------------------------------------
        ...
        # ----------------------------------------------------------
        # MATERIAL STATE (kg/s proxy interpretation)
        # ----------------------------------------------------------

        CaCO3_in = x["CaCO3"]

        # NOTE:
        # CaCO3 burada "inventory proxy" gibi kullanılıyor
        # consistent interpretation: kg/s equivalent flow
        ...
        heat_drive = np.tanh((T_gas - Ts_calc_curr) / 25.0)

        react_max_energy = CaCO3_in * heat_drive  # scaled proxy (dimensionless limiter)
        ...
        # ----------------------------------------------------------
        # SOFT BLENDING (STABILITY FIX)
        # ----------------------------------------------------------

        beta = 0.85
        reacted = beta * react_kin + (1 - beta) * react_max_energy
        ...
        # ----------------------------------------------------------
        # REACTION ENERGY (W = J/s)
        # ----------------------------------------------------------

        # NOTE: already per-step rate interpretation
        Q_reaction = reacted * DeltaH_calc

        # ----------------------------------------------------------
        # ENERGY BALANCE (SI CORRECTED)
        # ----------------------------------------------------------
        ...
        Ts_calc_next = (1 - alpha) * Ts_calc_curr + alpha * Ts_eq

        # reaction cooling effect (energy sink)
        Ts_calc_next -= (self.dt * Q_reaction / (C_solid + 1e-9)) * 0.3

        # ----------------------------------------------------------
        # SOFT LIMIT (NO HARD CLIP SHOCK)
        # ----------------------------------------------------------

        Ts_calc_next = 400.0 + (1600.0 - 400.0) / (
            1 + np.exp(-(Ts_calc_next - 1000.0) / 40.0)
        )
        ...
        Tg_calc_next = T_gas + 0.03 * (Ts_calc_curr - T_gas)
        ...
        if SiO2 <= 1e-9 or T_burn_eff < 1000.0:
            ...
            # stoichiometric limitation (mass closure)
            stoich_limit = min(
                SiO2 / 0.3488,
                CaO_pool / 0.6512,
            )

            # thermal activation (dimensionless)
            energy_factor = 1.0 / (1.0 + np.exp(-(T_burn_eff - 1100.0) / 80.0))

            dC2S = stoich_limit * kinetic * energy_factor

        # ----------------------------------------------------------
        # MASS UPDATE (CaO CONSUMPTION)
        # ----------------------------------------------------------
        ...
        # ----------------------------------------------------------
        # REACTION ENTHALPY (W = J/s CONSISTENT)
        # ----------------------------------------------------------

        DeltaH_c2s = 250.0 * 1000.0  # J per unit reaction extent

        Q_c2s = dC2S * DeltaH_c2s

        # ==========================================================
        # BURNING ZONE ENERGY FEEDBACK (STABLE COUPLING)
        # ==========================================================
        ...
        # lumped thermal inertia (J/K)
        C_burn_total = 6500.0 * Cp_s + 220.0 * Cp_g + 15000.0 * Cp_w

        # temperature equivalent impact
        dT_burn = Q_c2s / (C_burn_total + 1e-9)

        # distributed feedback (small gains = stability)
        ...
        # updated effective temperature (K)
        T_burn_eff_next = (
        ...
        # ==========================================================
        # C3S FORMATION (ENERGY + MASS + THERMALLY COUPLED)
        # ==========================================================

        C2S_in = x_next.get("C2S", x["C2S"])  # already updated by C2S block

        # ----------------------------------------------------------
        # THERMAL STATE (burning zone)
        # ----------------------------------------------------------
        ...
        if C2S_in <= 1e-9 or T_burn_eff < 1473.15:
            ...
            # stoichiometric constraint (mass closure)
            stoich_limit = min(
                C2S_in / 0.7544,
        ...
        DeltaH_c3s = 420.0 * 1000.0  # J per reaction extent

        Q_c3s = dC3S * DeltaH_c3s

        # ==========================================================
        # BURNING ZONE THERMAL FEEDBACK (STABLE COUPLING)
        # ==========================================================
        ...
        # temperature perturbation (K equivalent)
        dT_burn_c3s = Q_c3s / (C_burn_total + 1e-9)

        # slightly stronger coupling than C2S (but still stable)
        Ts_burning_pred = Ts_burning_pred + 0.06 * dT_burn_c3s
        Tg_burning_pred = Tg_burning_pred + 0.08 * dT_burn_c3s
        Tw_burning_pred = Tw_burning_pred + 0.04 * dT_burn_c3s
        ...
        DeltaH_c3a = 380.0 * 1000.0  # J per extent
        ...
        # ==========================================================
        # EFFECTIVE TEMPERATURE UPDATE (K DOMAIN)
        # ==========================================================
        ...
        if Fe2O3_in > 1e-6 and T_burn_eff >= 1100.0:
            ...
            # ==========================================================
            # ENERGY TERM (lumped enthalpy)
            # ==========================================================
            DeltaH_c4af = 120.0 * 1000.0  # J per extent
        ...
        # ==========================================================
        # ENERGY FEEDBACK (burning zone coupling)
        # ==========================================================
        ...
        dT_burn_c4af = Q_c4af / (C_burn_total + 1e-9)

        Ts_burning_pred += 0.01 * dT_burn_c4af
        Tg_burning_pred += 0.015 * dT_burn_c4af
        Tw_burning_pred += 0.008 * dT_burn_c4af

        # ==========================================================
        # UPDATE EFFECTIVE BURNING TEMPERATURE (K DOMAIN)
        # ==========================================================
